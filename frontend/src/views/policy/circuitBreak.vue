...
const { t } = useI18n()
const columns = [
    { title: t('pages.circuitBreak.form.name'), dataIndex: 'name', width: 180 },
    {
        title: t('pages.circuitBreak.form.sourceApplicationId'),
        dataIndex: 'source_application_id',
        key: 'source_application_id',
        width: 150,
    },
    {
        title: t('pages.circuitBreak.form.targetServiceId'),
        dataIndex: 'target_service_id',
        key: 'target_service_id',
        width: 150,
    },
    { title: t('pages.circuitBreak.form.path'), dataIndex: 'path', width: 150 },
    { title: t('pages.circuitBreak.form.method'), dataIndex: 'method', width: 100 },
    {
    ...
    Modal.confirm({
        ...
        onOk: () => {
            return new Promise((resolve, reject) => {
                ;(async () => {
                    try {
                        const { success } = await apis.policy.delCircuitBreak(id).catch(() => {
                            throw new Error()
                        })
                        if (config('http.code.success') === success) {
                            resolve()
