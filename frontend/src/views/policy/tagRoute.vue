...
const { t } = useI18n()
const columns = [
    { title: t('pages.tagRoute.form.name'), dataIndex: 'name', width: 180 },
    {
        title: t('pages.tagRoute.form.sourceApplicationId'),
        dataIndex: 'source_application_id',
        key: 'source_application_id',
        width: 150,
    },
    {
        title: t('pages.tagRoute.form.targetServiceId'),
        dataIndex: 'target_service_id',
        key: 'target_service_id',
        width: 150,
    },
    { title: t('pages.tagRoute.form.path'), dataIndex: 'path', width: 150 },
    { title: t('pages.tagRoute.form.method'), dataIndex: 'method', width: 100 },
    { title: t('pages.tagRoute.form.enabled'), key: 'enabled', width: 80 },
    { title: t('pages.tagRoute.form.creator'), dataIndex: 'creator', width: 100 },
    { title: t('pages.tagRoute.form.description'), dataIndex: 'description', ellipsis: true },
    {
    ...
    Modal.confirm({
        ...
        onOk: () => {
            return new Promise((resolve, reject) => {
                ;(async () => {
                    try {
                        const { success } = await apis.policy.delRoute(id).catch(() => {
                            throw new Error()
                        })
                        if (config('http.code.success') === success) {
                            resolve()
