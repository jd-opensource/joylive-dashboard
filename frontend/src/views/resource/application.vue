...
const columns = [
    ...
    { title: t('pages.application.form.created_at'), key: 'created_at', fixed: 'right', width: 180 },
...
async function getPageList() {
    try {
        ...
        const { success, data, total } = await apis.application
            .getApplicationList({
                pageSize,
                current,
...
function handleRemove({ id }) {
    Modal.confirm({
        title: t('pages.application.delTip'),
        ...
        onOk: () => {
            return new Promise((resolve, reject) => {
                ;(async () => {
                    try {
                        const { success } = await apis.application.delApplication(id).catch(() => {
                            throw new Error()
                        })
                        if (config('http.code.success') === success) {
                            resolve()
